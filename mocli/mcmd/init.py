...
def update_lua_path(root, dist_path):
    ...
    # Update Lmod lua files
    cmd = (
        f'find {root} -type f -print0 | xargs -0 sed -i -e "s@{old_path}@{dist_path}@g"'
    )
    subprocess.run(cmd, shell=True, check=True)
...
class InstallLmod(Command):
    ...
    def arguments(subparsers):
        parser = subparsers.add_parser(
            InstallLmod.name, help="Install Lmod on your system"
        )
        parser.add_argument(
            "path", type=str, help="Base root location of the installation"
        )
        parser.add_argument(
            "--dist",
            type=str,
            default=None,
            help="Alternate location for binary distribution",
        )
        parser.add_argument(
            "--modules",
            type=str,
            default=None,
            help="Alternate location for module files",
        )
        parser.add_argument(
            "--arch", type=str, default=None, help="Architecture to install"
        )
